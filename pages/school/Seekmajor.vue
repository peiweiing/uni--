<template>
	<view class="page">
		...
		<view class="module two por">
			...
			<view class="FX-sb">
				<view class="FY-c" @tap="show(1)">
					<text :class="isActive ? 'active' : ''">专业类别</text>
				...
				<view class="FY-c" @tap="show(2)">
					<text :class="poas ? 'active' : ''">门类/学科</text>
			...
			<view class="poa w100 pone" v-if="(isActive==true||poas==true)">
				<view class="FX" style="min-height: calc(80vh);">
					<view class="w3 FY FY-c" v-if="isActive">
		...
		<view class="module three FX-sb FY-c" v-for="(v,i) in arrc">
			<view class="FX-sb FY-c left">
				<view class="FY">
					<text style="font-size: 16px;">{{v.name}}</text>
					<text style="color: #999;">专业编码：{{v.code}}</text>
				</view>
				<text style="color: #999;">学历层次：{{v.txt}}</text>
			</view>
			<view class="FY FY-fe right">
				<image class="img" src="../../static/images/index/icon_next_3x.png" mode=""></image>
			</view>
		</view>
	...
	export default {
		data() {
			return {
				...
				seles:0,
				arrc:[
					{name:'经济管理学',code:'020155',txt:'本科'},
					{name:'清华大学',code:'020155',txt:'本科'},
					{name:'经济管理学',code:'020155',txt:'本科'},
					{name:'清华大学',code:'020155',txt:'本科'},
					{name:'经济管理学',code:'020155',txt:'本科'},
				],
...
<style>
	...
	.three .left>view>text:last-child{
		margin-top: 10%;
	}
	.three .left .img{
		width: 100rpx;
